            ...
            <form>
                <div class="shadow sm:rounded-md sm:overflow-hidden">
                    <div class="px-4 py-3 space-y-6 bg-white sm:p-6">
                        <div class="font-semibold">Tambah Data Kelas</div>
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <select id="jurusan" wire:model='jurusanId'
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5">
                                    <option>Pilih Jurusan</option>
                                    @foreach($jurusan as $j)
                                        <option {{ $jurusanId == $j->id ? 'selected' : '' }} value="{{ $j->id }}">{{ $j->nama }}</option>
                                    @endforeach
                                </select>
                                @error('jurusanId')
                                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <input type="text" wire:model='semester' id="semester"
                                       class="w-full mt-1 border-gray-300 rounded-md shadow-sm form-input focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('judul') border-red-500 @enderror"
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            <div class="col-span-6 sm:col-span-3">
                                <label for="tahunAjaran" class="block text-sm font-medium text-gray-700">Tahun
                                    Ajaran</label>
                                <input type="text" wire:model='tahunAjaran' id="tahunAjaran"
                                       class="w-full mt-1 border-gray-300 rounded-md shadow-sm form-input focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('judul') border-red-500 @enderror"
                                       value="{{ old('tahunAjaran') ?? $tahunAjaran }}">
                                @error('tahunAjaran')
                                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="px-4 py-3 text-right bg-gray-50 sm:px-6">
                        <button type="button" wire:click="updateKelasModalClose()"
                        ...
                        <button type="button" wire:click.prevent="update({{ $kelasId }})"
